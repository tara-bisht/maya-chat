...
Coming-soon players live in a static catalog (`apps/web/lib/marketplace/bill.ts`), not `public.agents`. Putting them in the table would make them chat-able or force a status column and compiler guards; a public `select` on `agents` would also risk leaking `system_prompt` without a view. Live posters today still use a hardcoded `COMPANY` snapshot; catalog-backing that wall is [`TODO-006`](../backlog/ideas/006-catalog-driven-first-party-bill.md). Gallery remains the authenticated, DB-backed wall.
