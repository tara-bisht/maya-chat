...
export default async function PlanPage({
  ...
  searchParams: Promise<{ reason?: string | string[]; checkout?: string | string[] }>;
}) {
  ...
  const flash = parseCheckoutFlash(
    typeof checkout === "string" ? checkout : null,
  );
    ...
    <main className="py-6 md:py-10">
      <PageInner>
        <SectionKicker kicker={COPY.plan} title={heading ?? SEATS_KICKER.title}>
          {heading ? "Compare plans, then check out." : SEATS_KICKER.body}
        </SectionKicker>
        {flash ? (
          <p className="mb-6 max-w-xl font-sans text-sm text-ink-soft">
            {flash === "success"
