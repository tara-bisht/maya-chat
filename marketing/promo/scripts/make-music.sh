...
# no licensing, no downloads. Re-run to regenerate public/audio/muse/music.mp3.
set -euo pipefail
cd "$(dirname "$0")/.."
...
ffmpeg -y \
  ...
  -c:a libmp3lame -b:a 96k public/audio/muse/music.mp3
ffprobe -v error -show_entries format=duration -of csv=p=0 public/audio/muse/music.mp3
